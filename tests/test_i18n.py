...
def test_switching_languages_preserves_choices_and_results():
    ...
    assert any('가상 데이터 데모' in message.value for message in app.warning)
    ...
    assert app.selectbox(key='Data source::ko').value=='Synthetic demo'
    assert '가상 데이터 데모' in app.selectbox(key='Data source::ko').options
