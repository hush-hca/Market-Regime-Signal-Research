...
## Deployment fallback repair

The previous fallback existed only under ignored `data/` and was omitted from the Docker image. A deployment with blocked API access therefore had neither source. The verified historical Bybit snapshot is now committed separately under `bootstrap/`, allowed through `.dockerignore`, and explicitly copied by `Dockerfile.vercel`. The feed tries a local verified snapshot, then the bundled archive. Integrity failures and synthetic metadata are rejected; API failure details are logged and missing-data errors retain their underlying cause. An offline test runs from a directory with no local data to exercise the deployed failure path. This repair provides historical data availability; it does not establish live API access from the hosting region.
